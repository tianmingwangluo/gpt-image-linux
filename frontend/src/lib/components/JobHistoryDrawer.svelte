...
{#if open}
  <div class="fixed inset-0 z-50">
    <button class="drawer-backdrop absolute inset-0" type="button" aria-label="Close jobs" on:click={onClose}></button>
    <aside class="fade-in absolute right-0 top-0 flex h-full w-full max-w-lg flex-col border-l border-zinc-800 bg-zinc-900 shadow-2xl">
      <div class="flex items-center justify-between border-b border-zinc-800 p-5">
        <div class="min-w-0">
          <h2 class="text-lg font-semibold text-zinc-100">Job History</h2>
          <p class="mt-1 text-xs text-zinc-500">Queued and running generation jobs</p>
        </div>
      ...
      <div class="min-h-0 flex-1 overflow-y-auto p-5">
        {#if jobs.length === 0}
          <div class="rounded-xl border border-dashed border-zinc-800 bg-zinc-950/35 px-4 py-10 text-center">
            <p class="text-sm font-medium text-zinc-300">No running jobs</p>
            <p class="mt-2 text-xs text-zinc-500">Queued and running jobs will show here.</p>
          </div>
        {:else}
          <div class="space-y-3">
            {#each jobs as job}
              <label class="flex gap-3 rounded-xl border border-zinc-800 bg-zinc-950/45 p-4">
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center justify-between gap-3">
                    <span class="rounded-md border border-zinc-700 px-2 py-0.5 text-[11px] text-zinc-400">{job.operation || 'generation'}</span>
                    <span class="text-xs font-medium text-emerald-300">{job.status}</span>
                  </div>
                  <p class="mt-2 truncate text-sm text-zinc-200">{job.prompt || 'Untitled job'}</p>
                  <p class="mt-1 truncate text-xs text-zinc-500">{stageLabel(job)}</p>
                </div>
              </label>
            {/each}
          </div>
