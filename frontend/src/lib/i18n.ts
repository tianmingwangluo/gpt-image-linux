...
const en = {
  ...
  gallery: {
    ...
    exportPreparing: 'Preparing ZIP stream. The browser will save it when ready.',
    downloadingSelected: 'Downloading selected images',
    downloadPreparing: (count: number) => `Preparing ${count} selected image${count === 1 ? '' : 's'}.`,
    browserSavingDownload: 'Receiving ZIP stream. The browser save dialog may appear after download completes.',
    notInterruptible: 'Keep open',
...
const zh: TranslationSchema<typeof en> = {
  ...
  gallery: {
    ...
    exportPreparing: '正在准备 ZIP 流。完成后浏览器会保存文件。',
    downloadingSelected: '正在下载所选图片',
    downloadPreparing: (count) => `正在准备 ${count} 张所选图片。`,
    browserSavingDownload: '正在接收 ZIP 流。下载完成后浏览器会保存文件。',
    notInterruptible: '保持页面打开',
